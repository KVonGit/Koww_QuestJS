...
class Cmd {
  ...
  matchItem(str, cmdParams) {
    /* MODIFIED TO CATCH ALL */
    parser.currentCommand = this
    parser.currentCommand.all = false;
    if (str == 'all'){
      parser.currentCommand.all = true
    }
    /* END OF MOD*/
    let score = 0;
  ...
  noobjecterror(s) {
    if (typeof game.unrecognizedObjects == 'undefined') game.unrecognizedObjects = []
    game.unrecognizedObjects.push(s)
