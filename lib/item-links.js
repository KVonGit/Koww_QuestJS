...
itemLinks.getVerbsLinks = function (obj) {
  ...
  for (const o of verbArr) {
    s +=
      '<span class="list-link-verb" onclick="itemLinks.itemLinkClick(this)" ';
    s += `data-linkverb="${o}" data-objname="${obj.name}" data-objalias="${
      obj.alias
    }">${sentenceCase(o)}</span>`;
  }
  return s;
...
itemLinks.itemClick = function (el, event) {
...
itemLinks.updateItemLinks2 = function (el) {
  ...
  for (const key in el.dataset)
    options[key] = util.guessMyType(el.dataset[key]);
...
tp.text_processors.exitsHere = function (arr, params) {
  ...
  const list = w[player.loc]
    .getExitDirs()
    .map(
      (el) =>
        '<span class="exit-link" onclick="runCmd(\'' +
        el +
        "')\">" +
        el +
        "</span>"
  ...
  return formatList(list, {
    lastSep: lang.list_or,
    nothing: lang.list_nowhere,
  });
};
...
settings.nameTransformer = function (alias, obj, options = {}) {
  ...
  if (options.capitalise) alias = sentenceCase(alias)
